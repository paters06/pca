...
contains
    ...
    subroutine calculate_1D_jacobian(Pw_array, U_array, p, u, jacob)
        use nurbs_curve
        use utils
        integer, intent(in) :: p
        real, intent(in) :: u
        ...
        integer :: d
        real, dimension(2) :: tang_vec, grad
        ...
        jacob = sqrt((tang_vec(1)**2) + (tang_vec(2)**2))
    ...
    subroutine element_matrix_stiffness(Pw_array, U_array, p, apt, cpt, intg_pts, gauss_weights, &
        ...
        real :: u, jacob, detJac2, deltaU, wJac
        ...
        allocate(Kmat_i(element_nonzero_B_values))
        allocate(Bmat_i(element_nonzero_B_values))
        allocate(global_dofs_k(element_nonzero_B_values))
        allocate(global_dofs_l(element_nonzero_B_values))
        ...
        integration_loop: do j = 1, num_gauss_pts
            ...
            call calculate_1D_jacobian(Pw_array, U_array, p, u, jacob)
            call calculate_parametric_gradient(apt, cpt, detJac2)
            ! call print_real(jacob)
            wJac = jacob*detJac2*gauss_weights(j)
            Bmat_i = 0.
            ...
            row_loop: do l = 0, p
                column_loop: do k = 0, p
                    ...
                    ! Kmat_i(id_K_elem_i) = (1./(jacob**2))*nders(d+1,k+1)*nders(d+1,l+1)*jacob*detJac2*gauss_weights(j)
                    Bmat_i(id_K_elem_i) = nders(d+1,k+1)*nders(d+1,l+1)
                    id_K_elem_i = id_K_elem_i + 1
                end do column_loop
            end do row_loop
            ! print *, "======================"
            Kmat_i = Kmat_i + (1./(jacob**2))*Bmat_i*wJac
    ...
    subroutine assemble_weak_form(P_array, w_array, U_array, U_reduced, p, num_gauss_pts, young, inertia, load, id_load, Kmat, Fvec)
        ...
        real, dimension(:,:), allocatable :: Kred, Fred, Ured, Pw_array
        integer, dimension(:), allocatable :: global_dofs_k, global_dofs_l
        real :: u, total_length, length_i, jacob
